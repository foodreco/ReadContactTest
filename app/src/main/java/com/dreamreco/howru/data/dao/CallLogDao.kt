...
import com.dreamreco.howru.group.GetDDCClass
import com.dreamreco.howru.recommendationLogic.CallLogDataForTendency
import com.dreamreco.howru.recommendationLogic.CallLogDataForTendencyMinimal
import com.dreamreco.howru.recommendationLogic.CallLogDataForTendencyMinimal2
...
interface CallLogDao {
    ...
    @Query("SELECT date, duration FROM call_history WHERE number LIKE :number ORDER BY date DESC LIMIT 1")
    suspend fun getDDC(number: String): GetDDCClass?
    ...
    @Query("SELECT number FROM call_history WHERE number LIKE :number AND date LIKE :date AND duration LIKE :duration LIMIT 1")
    suspend fun confirmAndInsert(number: String, date: String, duration: String): String?
